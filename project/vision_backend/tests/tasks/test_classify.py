...
@mock.patch('images.models.Image.valset', MockImage.valset)
class ClassifyUnconfirmedImageRepeatedTest(BaseClassifyTest):

    @classmethod
    def setUpTestData(cls):
        super(ClassifyUnconfirmedImageRepeatedTest, cls).setUpTestData()
        # Provide enough data for training
        for _ in range(spacer_config.MIN_TRAINIMAGES):
            img = cls.upload_image(
                cls.user, cls.source, image_options=dict(filename='train{}.png'.format(cls.image_count)))
            cls.add_annotations(
                cls.user, img, {1: 'A', 2: 'B', 3: 'A', 4: 'A', 5: 'B'})
        for _ in range(1):
            img = cls.upload_image(
                cls.user, cls.source, image_options=dict(filename='val{}.png'.format(cls.image_count)))
            cls.add_annotations(
                cls.user, img, {1: 'A', 2: 'B', 3: 'A', 4: 'A', 5: 'B'})
        # Process feature extraction results
        collect_all_jobs()
